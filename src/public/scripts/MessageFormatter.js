...
function formatMessage(chat, message) {
    ...
    return (
    <View>
        <View style={chat.props.styles.messageContent}>
            <Text style={chat.props.styles.messageText}>{messageFormatted}</Text>
            {
            message.edited ? <Text style={chat.props.styles.messageEditedText}> (edited)</Text> : null
            }
        </View>
    </View>
    );
